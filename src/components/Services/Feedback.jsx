...
export default function Feedback() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:2023/api/feedback/feedback")
      .then((response) => {
  ...
  const markFeedbackAsSeen = async (id) => {
    try {
      await axios.put(`http://localhost:2023/api/feedback/feedback/${id}/mark-seen`);
  ...
  const markFeedbackAsUnseen = async (id) => {
    try {
      await axios.put(`http://localhost:2023/api/feedback/feedback/${id}/mark-unseen`);
      const updatedFeedbackList = feedbackList.map((feedback) => {
        if (feedback._id === id) {
      ...
      <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <tbody>
          ...
          {feedbackList.map((feedback) => (
            <tr key={feedback.id}>
              <td className="px-6 py-4 whitespace-no-wrap">{feedback.firstName}</td>
              <td className="px-6 py-4 whitespace-no-wrap">{feedback.lastName}</td>
